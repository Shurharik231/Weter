...
def _segments_intersect(a: GeoPoint, b: GeoPoint, c: GeoPoint, d: GeoPoint) -> bool:
    ...
    def on_segment(p, q, r):
        return min(p.lon,r.lon)-1e-12 <= q.lon <= max(p.lon,r.lon)+1e-12 and min(p.lat,r.lat)-1e-12 <= q.lat <= max(p.lat,r.lat)+1e-12
...
def _segment_circle_hit(a: GeoPoint, b: GeoPoint, center: GeoPoint, radius_m: float) -> tuple[bool,float]:
    """Return whether a trajectory segment intersects a target circle and an approximate inside fraction."""
    lat0=radians(center.lat); scale_x=EARTH_RADIUS_M*cos(lat0); scale_y=EARTH_RADIUS_M
    ax=(radians(a.lon)-radians(center.lon))*scale_x; ay=(radians(a.lat)-radians(center.lat))*scale_y
    bx=(radians(b.lon)-radians(center.lon))*scale_x; by=(radians(b.lat)-radians(center.lat))*scale_y
    dx,dy=bx-ax,by-ay; aa=dx*dx+dy*dy
    if aa < 1e-12:
        return (ax*ax+ay*ay <= radius_m*radius_m, 1.0 if ax*ax+ay*ay <= radius_m*radius_m else 0.0)
    t=max(0.0,min(1.0,-(ax*dx+ay*dy)/aa)); px,py=ax+t*dx,ay+t*dy
    dist2=px*px+py*py
    if dist2 > radius_m*radius_m: return False,0.0
    half=sqrt(max(0.0,radius_m*radius_m-dist2)/aa)
    lo=max(0.0,t-half); hi=min(1.0,t+half)
    return True,max(0.0,hi-lo)
...
def _segment_target_hit(a: TrajectoryPoint, b: TrajectoryPoint, target: GeoPoint | None, radius_m: float, polygon: list[GeoPoint] | None) -> tuple[bool,float]:
    ...
    if polygon and len(polygon)>=3:
        ...
        return False,0.0
    if target is None: return False,0.0
    return _segment_circle_hit(pa,pb,target,radius_m)
...
def _trajectory_quality(points: list[TrajectoryPoint], target: GeoPoint | None, radius_m: float, target_polygon: list[GeoPoint] | None = None) -> tuple[float, TrajectoryPoint, float, bool]:
    ...
    for a,b in zip(points,points[1:]):
        ...
        segment_hit,fraction=_segment_target_hit(a,b,target,radius_m,target_polygon)
        ...
        if ia and ib: inside+=dt; hit=True
            ...
            inside+=dt*max(0.0,min(1.0,fraction)); hit=True
    if target_polygon and len(target_polygon)>=3:
        hit=hit or _trajectory_hits_polygon(points,target_polygon)
...
def _weather_hazard(forecast: dict[str, Any], points: list[TrajectoryPoint], precipitation_limit_mm: float) -> bool:
    ...
    for p in points:
        ...
        p_time=_utc(p.time);parsed_times=[_utc(datetime.fromisoformat(t.replace("Z","+00:00"))) for t in times]
...
def _perturb_forecast(forecast: dict[str, Any], seed: int, wind_sigma_mps: float) -> dict[str, Any]:
    ...
    for cell in result.get("cells",[]):
        ...
        for level in [k[len("wind_speed_"):] for k in hourly if k.startswith("wind_speed_")]:
            sk,dk=f"wind_speed_{level}",f"wind_direction_{level}";speeds,directions=hourly.get(sk),hourly.get(dk)
            if not speeds or not directions:continue
            for i,(s,d) in enumerate(zip(speeds,directions)):
                if s is None or d is None:continue
                sigma=wind_sigma_mps*(1.0+0.35*sqrt(max(0.0,i)/24.0));common_e=0.82*common_e+rng.gauss(0,sigma*0.30);common_n=0.82*common_n+rng.gauss(0,sigma*0.30);series_e=0.88*series_e+rng.gauss(0,sigma*0.55);series_n=0.88*series_n+rng.gauss(0,sigma*0.55)
                towards=radians((float(d)+180)%360);u=float(s)*sin(towards)+common_e+series_e;v=float(s)*cos(towards)+common_n+series_n;speed=sqrt(u*u+v*v)
                directions[i]=(degrees(atan2(u,v))+180)%360 if speed>1e-9 else 0.0;speeds[i]=speed
    return result
...
def _ensemble_metrics(params: FlightParameters, base_forecast: dict[str, Any], target: GeoPoint | None, target_polygon: list[GeoPoint] | None, radius_m: float, members: int, wind_sigma_mps: float, restricted: list[list[GeoPoint]], hazards: list[list[GeoPoint]], progress: Callable[[int],None]|None=None) -> tuple[float,float,float]:
    ...
    for member in range(max(1,members)):
        ...
        restricted_hit=any(_trajectory_hits_polygon(trajectory.points,poly) for poly in restricted);hazard_hit=any(_trajectory_hits_polygon(trajectory.points,poly) for poly in hazards)
...
def find_favorable_windows(*, target: GeoPoint | None, target_polygon: list[GeoPoint] | None=None, launch_center: GeoPoint, forecast: dict[str, Any], search_start: datetime, search_end: datetime, launch_radius_m: float, target_radius_m: float, time_step_hours: float, duration_hours: float, step_minutes: float, ascent_rate: float, max_altitude: float, start_altitude: float, launch_points_rings: int, launch_points_per_ring: int, surface_wind_limit_mps: float, shear_limit_s_inv: float, ensemble_members: int, ensemble_wind_sigma_mps: float, precipitation_limit_mm: float, restricted_zones: list[list[GeoPoint]]|None=None, hazard_zones: list[list[GeoPoint]]|None=None, progress_callback: ProgressCallback|None=None) -> list[FavorableWindow]:
    ...
    for t in times:
        for launch in points:
            ...
            restricted_hit=any(_trajectory_hits_polygon(trajectory.points,z) for z in restricted_zones);hazard_hit=_weather_hazard(forecast,trajectory.points,precipitation_limit_mm) or any(_trajectory_hits_polygon(trajectory.points,z) for z in hazard_zones)
    ...
    if not candidates:
        ...
        return []
    candidates.sort(key=lambda c:c.score);target_count=min(len(candidates),max(24,int(len(candidates)*0.20)));selected=candidates[:target_count]
    best_per_time={}
    ...
    for idx,c in enumerate(selected,1):
        ...
        c.score=c.ensemble_p90_distance_m/max(100.0,target_radius_m)-c.ensemble_success_rate*2.0+c.min_distance_m/max(100.0,target_radius_m);final.append(c)
    ...
    for c in final:
        if not windows or _seconds_between(c.launch_time,windows[-1].window_end)>3600:windows.append(FavorableWindow(c.launch_time,c.launch_time,c.launch_time,[c]))
            ...
            windows[-1].window_end=max(windows[-1].window_end,c.launch_time);windows[-1].candidates.append(c);windows[-1].recommended_time=min(windows[-1].candidates,key=lambda x:x.score).launch_time
