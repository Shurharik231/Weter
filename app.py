...
app = FastAPI(title="Wind Trajectory", version="3.5.0")
...
def trajectory_grid_radius_km(duration_hours: float) -> float:
    # Conservative field around the launch/target corridor; the trajectory integrator clamps only at the field boundary.
    return min(4000.0, max(400.0, 400.0 + duration_hours * 45.0))
...
async def create_trajectory(request: TrajectoryRequest):
    try:
        start_time = ensure_utc(request.start_time); end_time = start_time + timedelta(hours=request.duration_hours)
...
async def create_backtrajectory(request: BackTrajectoryRequest):
    try:
        ...
        weather = OpenMeteoProvider(workdir=BASE_DIR / "weather_cache")
        ...
        result = estimator.run(detection); representative = result.trajectories[0] if result.trajectories else []
...
@app.get("/api/config")
async def config():
    return {"max_trajectory_hours":168,"default_step_minutes":10,"favorable":{"ensemble":True,"correlated_uncertainty":True,"surface_wind_filter":True,"wind_shear_filter":True,"weather_hazard_filter":True,"restricted_zones":True,"target_polygon":True,"ensemble_p90":True}}