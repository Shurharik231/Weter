...
def serialize_candidate(c: LaunchCandidate) -> dict:
    return {
        ...
        "trajectory": {
            ...
            "points": [
                {
                    "time": p.time.isoformat(), "lat": p.lat, "lon": p.lon,
...
async def get_wind(lat: float, lon: float, altitude: float = 1000):
    try:
        ...
        forecast = await fetch_grid(latitudes=latitudes, longitudes=longitudes, start=now, end=now)
        speed, direction = interpolate_wind(forecast=forecast, lat=lat, lon=lon, altitude=altitude, when=now)
...
async def create_trajectory(request: TrajectoryRequest):
    try:
        ...
        parameters = FlightParameters(
            start=StartPoint(lat=request.start.lat, lon=request.start.lon),
            start_time=start_time, start_altitude=request.start_altitude,
            ascent_rate=request.ascent_rate, max_altitude=request.max_altitude,
            duration_hours=request.duration_hours, step_minutes=request.step_minutes,
        )
        return calculate_trajectory(params=parameters, forecast=forecast)
...
async def create_backtrajectory(request: BackTrajectoryRequest):
    try:
        ...
        step_seconds = float((request.step_minutes or 1) * 60)
        ascent_rate_mps = request.ascent_rate or 5.0
...
async def create_favorable(request: FavorableRequest):
    try:
        ...
        radius = grid_radius_for_duration(request.duration_hours) + 100
