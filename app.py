...
from backtrajectory import BalloonModel, Detection, OpenMeteoProvider, EnsembleEstimator, BackTrajectorySolver
...
app = FastAPI(title="Wind Trajectory", version="3.4.0")
...
class BackTrajectoryRequest(BaseModel):
    ...
    members: int = Field(100, ge=20, le=250)
...
class FavorableRequest(BaseModel):
    ...
    duration_hours: float = Field(4, gt=0, le=48)
    ...
    start_altitude: float = Field(0, ge=0)
    launch_points_rings: int = Field(2, ge=1, le=5)
    launch_points_per_ring: int = Field(12, ge=8, le=36)
    surface_wind_limit_mps: float = Field(10, ge=0, le=50)
    shear_limit_s_inv: float = Field(0.002, gt=0, le=0.02)
    ensemble_members: int = Field(25, ge=1, le=100)
...
def trajectory_grid_radius_km(duration_hours: float) -> float:
    # Larger horizons need a wider meteorological field.
    return min(2500.0, max(300.0, 350.0 + duration_hours * 35.0))
...
def serialize_window(window) -> dict:
    ...
    return {
        ...
        "recommended_time": window.recommended_time.isoformat(),
...
async def create_backtrajectory(request: BackTrajectoryRequest):
    try:
        ...
        workdir = BASE_DIR / "weather_cache"
        weather = OpenMeteoProvider(workdir=workdir)
        ...
        solver = BackTrajectorySolver(weather=weather, balloon=balloon, step_seconds=float((request.step_minutes or 1)*60), max_duration_hours=duration_hours)
...
async def create_favorable(request: FavorableRequest):
    try:
        ...
        if end <= start:
            raise HTTPException(status_code=400, detail="search_end must be after search_start")
        radius = trajectory_grid_radius_km(request.duration_hours) + request.launch_radius_m / 1000.0
        center_lat = (request.launch_center.lat + request.target.lat) / 2
        center_lon = (request.launch_center.lon + request.target.lon) / 2
        ...
        windows = find_favorable_windows(
            target=GeoPoint(request.target.lat, request.target.lon),
            ...
            precipitation_limit_mm=request.precipitation_limit_mm, restricted_zones=restricted, hazard_zones=hazards,
        )
        ...
        raise HTTPException(status_code=502, detail=f"Favorable search/weather error: {safe_detail(exc)}") from exc

@app.get("/api/config")
async def config():
    return {"max_trajectory_hours":168,"default_step_minutes":10,"favorable":{"ensemble":True,"surface_wind_filter":True,"wind_shear_filter":True,"weather_hazard_filter":True,"restricted_zones":True}}
