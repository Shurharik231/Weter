...
function updateFavTargetMarker(lat, lon) {
    ...
    if (favTargetCircle) {
        ...
        favTargetCircle = L.circle([lat, lon], { radius: radius, color: "#dc2626", fillColor: "#dc2626", fillOpacity: 0.08, weight: 2, dashArray: "6 5" }).addTo(map);
    }
}
...
function updateFavLaunchCircle() {
    ...
    if (favLaunchCircle) {
        ...
        favLaunchCircle = L.circle(p, { radius: radius, color: "#16a34a", fillColor: "#16a34a", fillOpacity: 0.09, weight: 2 }).addTo(map);
    }
}

function clearFavorableMap() {
    if (!favMapReady()) return;
    if (favTrajectoryLayer) { map.removeLayer(favTrajectoryLayer); favTrajectoryLayer = null; }
}

function setFavorableMode() {
    var directTab = favEl("tab-direct"), backTab = favEl("tab-back"), favTab = favEl("tab-favorable");
    var directPanel = favEl("direct-panel"), backPanel = favEl("back-panel"), favPanel = favEl("favorable-panel"), mapMode = favEl("map-mode");
...
function setFavorableStatus(text, error) {
    var status = favEl("status"); if (!status) return;
...
function setFavSearchDefaults() {
    var start = favEl("fav-search-start"), end = favEl("fav-search-end"); if (!start || !end) return;
    var now = new Date(), later = new Date(now.getTime() + 72 * 3600000);
...
function renderFavorableMap(data) {
    if (!favMapReady()) return;
    clearFavorableMap();
    var group = L.layerGroup().addTo(map); favTrajectoryLayer = group; var bounds = [], windows = data.windows || [];
    windows.forEach(function(windowItem, wi) {
        (windowItem.best_candidates || []).forEach(function(candidate, ci) {
            var points = candidate.trajectory && candidate.trajectory.points || []; if (points.length < 2) return;
            var latlngs = points.map(function(p) { return [p.lat, p.lon]; }); latlngs.forEach(function(p) { bounds.push(p); });
            var isBest = wi === 0 && ci === 0;
            var line = L.polyline(latlngs, { color: isBest ? "#2563eb" : "#7c3aed", weight: isBest ? 5 : 2, opacity: isBest ? 0.95 : 0.25, dashArray: isBest ? null : "6 8" }).addTo(group);
            line.bindTooltip(isBest ? "Рекомендуемая траектория" : "Возможный сценарий", { sticky: true });
            line.bindPopup("Окно " + (wi+1) + "<br>Запуск: " + new Date(candidate.launch_time).toLocaleString("ru-RU") + "<br>Мин. расстояние: " + Math.round(candidate.min_distance_to_target_m) + " м" + "<br>В цели: " + (Number(candidate.time_in_target_s || 0)/60).toFixed(1) + " мин");
        });
    });
...
function renderFavorableResults(data) {
    var results=favEl("results"), content=favEl("result-content"); if(results) results.classList.remove("hidden"); var windows=data.windows||[];
    if(!windows.length){ if(content) content.innerHTML="<div class='result-card'>Подходящих окон не найдено.</div>"; return; }
    if(!content) return;
    content.innerHTML=windows.map(function(w,i){
        var best=(w.best_candidates||[])[0]; if(!best) return ""; var closest=best.closest_point||{}, success=best.ensemble_success_rate, weather=best.weather||{};
        return '<div class="result-card"><strong>Окно #'+(i+1)+'</strong><div><b>Период:</b> '+new Date(w.window_start).toLocaleString("ru-RU")+' — '+new Date(w.window_end).toLocaleString("ru-RU")+'</div><div><b>Рекомендуемый запуск:</b> '+new Date(w.recommended_time).toLocaleString("ru-RU")+'</div><div><b>Точка:</b> '+Number(best.launch_point.lat).toFixed(5)+', '+Number(best.launch_point.lon).toFixed(5)+'</div><div><b>Минимум до цели:</b> '+formatMeters(best.min_distance_to_target_m)+'</div><div><b>В целевой области:</b> '+(Number(best.time_in_target_s||0)/60).toFixed(1)+' мин</div><div><b>Максимальное сближение:</b> '+(closest.time?new Date(closest.time).toLocaleString("ru-RU"):"—")+(closest.altitude!=null?' на высоте '+Math.round(closest.altitude)+' м':'')+'</div>'+(success!=null?'<div><b>Успешность ансамбля:</b> '+(Number(success)*100).toFixed(0)+'%</div>':'')+(weather.surface_wind_speed_mps!=null?'<div><b>Приземный ветер:</b> '+Number(weather.surface_wind_speed_mps).toFixed(1)+' м/с</div>':'')+'</div>';
    }).join("");
}
...
async function calculateFavorable() {
    ...
    var button=favEl("calculate-favorable"); if(button) button.disabled=true; clearFavorableMap(); setFavorableStatus("Получаем прогноз ветра и ищем варианты запуска…");
    try {
        ...
        var response=await fetch("/api/favorable",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}); var data=await response.json(); if(!response.ok) throw new Error(data.detail||("HTTP "+response.status));
        favResultData=data; renderFavorableResults(data); renderFavorableMap(data);
        setFavorableStatus(data.windows&&data.windows.length?"Готово. Выберите окно в результатах.":"Расчёт завершён: подходящих окон не найдено.");
    } catch(error){console.error(error);setFavorableStatus("Ошибка расчёта: "+error.message,true);} finally {if(button) button.disabled=false;}
}

function initFavorableControls() {
    var tab=favEl("tab-favorable"), target=favEl("fav-set-target"), launch=favEl("fav-set-launch"), calc=favEl("calculate-favorable"), radius=favEl("fav-radius"), maxDist=favEl("fav-max-dist");
    if(tab) tab.addEventListener("click",setFavorableMode); if(target) target.addEventListener("click",enableFavTargetSelection); if(launch) launch.addEventListener("click",enableFavLaunchSelection); if(calc) calc.addEventListener("click",calculateFavorable);
    if(radius) radius.addEventListener("input",updateFavLaunchCircle); if(maxDist) maxDist.addEventListener("input",function(){var lat=Number(favTargetLat?.value),lon=Number(favTargetLon?.value);if(validFavCoordinates(lat,lon))updateFavTargetMarker(lat,lon);});
    setFavSearchDefaults();
}
