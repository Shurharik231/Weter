...
PRESSURE_LEVELS = [(1000,"1000hPa"),(975,"975hPa"),(950,"950hPa"),(900,"900hPa"),(850,"850hPa"),(800,"800hPa"),(700,"700hPa"),(600,"600hPa"),(550,"550hPa"),(500,"500hPa"),(450,"450hPa"),(400,"400hPa"),(350,"350hPa"),(300,"300hPa")]
...
def _time_indices(times: list[datetime], target: datetime):
    if target <= times[0]: return 0,0,0.0
    if target >= times[-1]:
...
def _bilinear_wind(forecast, lat, lon, altitude, target_time):
    ...
    lat=max(lats[0],min(lats[-1],lat)); lon=max(lons[0],min(lons[-1],lon))
...
def wind_field(forecast, lat, lon, altitude, when, radius_km=150, count=9):
    latitudes,longitudes=build_grid(lat,lon,radius_km)
    return [{"lat":a,"lon":b,"speed":interpolate_wind(forecast,a,b,altitude,when)[0],"direction":interpolate_wind(forecast,a,b,altitude,when)[1]} for a in latitudes for b in longitudes]
