...
function installDocuments(){
  ...
  .weter-doc-modal{position:fixed;inset:0;width:100%;height:100%;height:100dvh;z-index:30000;background:rgba(15,23,42,.48);display:flex;align-items:center;justify-content:center;padding:16px;opacity:0;visibility:hidden;pointer-events:none;transition:opacity .2s ease,visibility .2s ease;overflow:hidden}
  .weter-doc-modal.open{opacity:1;visibility:visible;pointer-events:auto}
  .weter-doc-window{width:min(920px,100%);height:min(88vh,900px);height:min(88dvh,900px);max-height:calc(100dvh - 32px);background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 18px 60px #0005;transform:translateY(10px) scale(.985);transition:transform .22s ease;position:relative;display:flex;flex-direction:column;min-height:0}
  ...
  .weter-doc-close:hover{background:#eef1f4}.weter-doc-frame{display:block;flex:1 1 auto;width:100%;height:0;min-height:0;flex-basis:0;border:0;background:#fff;overflow:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain}
  @media(max-width:850px){
    ...
    .weter-doc-modal{padding:8px;align-items:center;overflow:hidden;overscroll-behavior:none}
    .weter-doc-window{width:100%;height:calc(100dvh - 16px);max-height:none;border-radius:12px;min-height:0}
