...
function showPoints(layer,points){
 ...
 points.forEach((p,i)=>{
   const marker=L.circleMarker([p.lat,p.lon],{radius:3,weight:1,fillOpacity:.9});
   marker.bindTooltip(`<b>Точка ${i+1}</b><br>Время: ${p.time?new Date(p.time).toLocaleString():'—'}<br>Высота: ${num(p,['altitude_m','altitude']).toFixed(0)} м<br>Скорость: ${num(p,['speed_mps','wind_speed_mps','ground_speed_mps']).toFixed(2)} м/с`,{direction:'top',sticky:true});
   marker.addTo(group);
 });
 group.addTo(map);layer._trajectoryPointMarkers=group;
}
const original=window.line;
if(typeof original!=='function')return;
window.line=function(points,color='#1f6feb',weight=3,title='Траектория'){
 const layer=original(points,color,weight,title);
 if(!layer||!points||points.length<2)return layer;
 layer.bindPopup(content(points,title),{maxWidth:460,autoPan:false,closeButton:true,closeOnClick:false});
 layer.on('mouseover',e=>{layer.setStyle({weight:Math.max(weight+2,5)});layer.openPopup(e.latlng)});
 layer.on('mouseout',()=>layer.setStyle({weight}));
 layer.on('click',e=>{layer.openPopup(e.latlng);showPoints(layer,points)});
 layer.on('remove',()=>{if(layer._trajectoryPointMarkers)map.removeLayer(layer._trajectoryPointMarkers)});
 return layer;
};
})();
